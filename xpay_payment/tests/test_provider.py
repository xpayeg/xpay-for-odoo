...
class TestProvider(XPayCommon):
    ...
    def test_action_refresh_account_activates_archived_methods_the_account_newly_lists(self):
        """XPay's own `instapay` and Odoo's own `bank_transfer` (and, as it
        happens, `valu`) all ship archived. The default active filter on
        `payment.method` must not hide them from the search that links
        them, and Odoo only un-archives on a disabled -> enabled/test
        `write()` transition (`_activate_default_pms`) — Refresh account
        does not go through that, so it must call the same mechanism
        directly for a method the account now lists to become usable."""
    ...
    def test_action_refresh_account_leaves_an_unlisted_archived_method_archived(self):
        tabby = self.env.ref("payment.payment_method_tabby")
        tabby.active = False
