...
class TestRefund(XPayCommon, PaymentHttpCommon):
    ...
    def test_refund_request_body_and_idempotency(self):
        ...
        refund_tx = tx._refund(amount_to_refund=250.0)
    ...
    def test_full_refund_without_a_presentment_mirror_sends_the_source_amount(self):
        ...
        refund_tx = tx._refund(amount_to_refund=750.0)
    ...
    def test_full_refund_with_a_presentment_mirror_sends_the_stored_processing_amount(self):
        ...
        refund_tx = tx._refund(amount_to_refund=110.00)
    ...
    def test_retry_after_a_lost_response_replays_the_same_key(self):
        ...
        first_attempt = tx._refund(amount_to_refund=100.0)
        ...
        retry = tx._refund(amount_to_refund=100.0)
    ...
    def test_submit_refund_refuses_without_an_api_call_when_a_sibling_refund_is_pending(self):
        ...
        pending_sibling = tx._refund(amount_to_refund=100.0)
        self.assertEqual(pending_sibling.state, "pending")
        calls_before = len(transport.calls)

        second = tx._refund(amount_to_refund=50.0)
    ...
    def test_a_locally_refused_attempt_never_counts_as_unsettled(self):
        ...
        other = tx._refund(amount_to_refund=50.0)
    ...
    def test_adoption_never_picks_a_never_sent_child(self):
        ...
        tx = self._paid_tx()
        transport.fail(XPayTransportError("response lost"))
        tx._refund(amount_to_refund=100.0)  # awaiting an answer
        refused = tx._refund(amount_to_refund=50.0)  # refused here, never sent
        self.assertFalse(refused.xpay_refund_awaiting_answer)
    ...
    def test_usd_presentment_partial_refund_converts_to_the_processing_currency(self):
        ...
        refund_tx = tx._refund(amount_to_refund=55.00)
    ...
    def test_a_second_partial_refund_completing_the_order_sends_the_exact_remainder(self):
        ...
        first = tx._refund(amount_to_refund=50.00)
        self.assertEqual(first.state, "done")
        ...
        second = tx._refund(amount_to_refund=60.00)
    ...
    def test_a_child_rejected_by_a_webhook_is_never_an_adoption_candidate_for_a_later_report(self):
        ...
        tx = self._paid_tx()
        transport.fail(XPayTransportError("response lost"))
        lost = tx._refund(amount_to_refund=100.0)  # stored processing amount 10000
    ...
    def test_adoption_never_picks_a_rejected_sibling(self):
        ...
        rejected = tx._refund(amount_to_refund=100.0)  # stored processing amount 10000
    ...
    def test_adoption_refuses_a_matching_amount_in_a_foreign_currency(self):
        ...
        transport.fail(XPayTransportError("response lost"))
        errored = tx._refund(amount_to_refund=100.0)  # stored processing amount 10000
    ...
    def test_refund_created_completes_a_pending_child_whose_presentment_echo_is_one_unit_off(
        ...
        # child's own amount. The framework's amount check would error the
        # pending child on a refund that succeeded; this module opts out of
        # it for refunds and checks the processing amount exactly instead.
        transport = self._use_transport()
        tx = self._paid_tx()
        tx.xpay_presentment_json = {
            "presentment_currency": "EGP",
        ...
        refund_tx = tx._refund(amount_to_refund=750.0)
        self.assertEqual(refund_tx.state, "pending")
        self.assertIsNone(refund_tx._extract_amount_data({"refund": {"amount": 1485}}))
    ...
    def _post_webhook(self, event):
        ...
        header = self.sign_header("whsec_test_1", body)
        url = self._build_url(f"/payment/xpay/webhook/{self.provider.id}")
        return self.url_open(url, data=body, headers={"XPay-Signature": header}, method="POST")
    ...
    def test_a_demo_provider_refund_completes_alongside_this_module(self):
        # The base method and every other provider's own override of
        # `_send_refund_request` take no argument, so this override must
        # not pass one either, or refunding any OTHER provider's
        # transaction raises a TypeError as long as this module is
        # installed, whether or not that transaction was ours.
        ...
        demo_refund_tx = demo_tx._refund(amount_to_refund=50.0)
        ...
        xpay_refund_tx = tx._refund(amount_to_refund=100.0)
