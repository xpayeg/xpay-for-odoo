...
class XPayMainController(http.Controller):
    ...
    def xpay_webhook(self, provider_id, **kwargs):
        ...
        tx = request.env["payment.transaction"].sudo()._search_by_reference("xpay", payment_data)
        ...
        if not tx and events.is_refund_event(event_type):
            ...
            # store has never seen. `_search_by_reference` stays a pure
            # lookup; the mirror-in-a-new-child creation lives here.
